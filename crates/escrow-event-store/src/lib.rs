...
pub use item::{Log, ReadModelRow, Recorded};
...
/// リードモデルの DDL。理由は `read_models/item.sql` に在る。
const READ_MODEL: &str = include_str!("../read_models/item.sql");

/// 項目の中でのイベントの通し番号。1 から始まり、1 は必ず `discovered`（#1）。
///
...
pub enum EventStoreError {
    ...
    #[error("項目 {0} が無い")]
    NoSuchItem(ItemId),
    /// 読んでから書くまでに、誰かがイベントを足していた。読み直して決め直す（#7）。
    #[error("読んだときからイベントが増えている。読み直して決め直す")]
    Superseded,
}
...
impl EventStore {
    ...
    pub async fn open(path: &Path) -> Result<Self, EventStoreError> {
        ...
        if let Some(parent) = path.parent() {
            std::fs::create_dir_all(parent).map_err(|source| EventStoreError::Io {
    ...
    /// テスト用。プロセスが終われば消える。
    pub async fn open_in_memory() -> Result<Self, EventStoreError> {
        let options = SqliteConnectOptions::new()
            .in_memory(true)
            .foreign_keys(true);
    ...
    async fn connect(options: SqliteConnectOptions) -> Result<Self, EventStoreError> {
        ...
        // リードモデルは移行に載らないので、無ければここで作る。DDL の写しは
        // read_models/item.sql の1つきりで、rebuild も同じものを流す。
        pool.execute(READ_MODEL).await?;
...
mod tests {
    ...
    use crate::{EventStore, EventStoreError, Seq};

    #[tokio::test]
    async fn migrations_apply_to_an_empty_database() {
        // open_in_memory が移行とリードモデルの作成まで通す。通らなければここで落ちる。
    ...
    async fn opening_an_existing_database_keeps_its_read_model() {
        ...
        let source = crate::testing::seed_into(&store).await;
        let id = crate::testing::a_holding_item(&store, source).await;
        drop(store);

        let store = EventStore::open(&path).await.unwrap();
        assert!(
            store.item(id).await.unwrap().is_some(),
            "リードモデルが残っている"
        );
    }
    ...
    async fn refuses_a_database_from_a_newer_binary() {
        ...
        match EventStore::open(&path).await {
            Err(EventStoreError::Migrate(sqlx::migrate::MigrateError::VersionMissing(999))) => {}
