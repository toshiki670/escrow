...
--
-- 入るのはイベントログと、人が編集する設定テーブルだけ。リードモデル（`item`）は入らない —
-- スキーマを変えたいときは移行を書かず `rebuild` で作り直すので、移行に混ぜると
-- いつの間にかリードモデルに手作業の移行が当たって「捨てて作り直せる」が約束にしか
-- ならなくなる（#15）。
--
...
--
-- `item_id` の**同一性と採番はこのテーブルが持ち、リードモデルとは独立**。リードモデルは
-- 捨てて作り直せるものなので、真実の側がそこへ外部キーを張ると、`rebuild` の DROP が
-- 連鎖してログごと消える。
--
...
CREATE TABLE item_event (
    ...
    -- イベントの判別子。対応の保証は Rust の enum で、CHECK 制約は置かない（#1）。
    kind        TEXT    NOT NULL,
    occurred_at TEXT    NOT NULL,

    -- ここから下は「その kind が運ばないなら NULL」。
    -- discovered — 項目の中身を丸ごと運ぶ。これが無いとリードモデルを作り直せない。
    url                TEXT,
    content_type       TEXT,
    published_at       TEXT,
