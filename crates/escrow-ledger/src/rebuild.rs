//! 投影を捨てて作り直す（#15）。
//!
//! 投影のスキーマを変えたいときは移行を書かず、これを走らせる。移行に混ぜると、
//! いつの間にか投影に手作業の移行が当たって作り直せなくなり、「捨てて作り直せる」が
//! 建前になる。
//!
//! #1 の「導出できるものは持たない」の唯一の例外が投影で、**いつでも捨てて
//! 作り直せることがその条件**。作り直す手段が実装されていない投影は、ただの重複。
...
mod tests {
    ...
    ///
    /// 公開 API に投影を書き換える口が無いので、**壊す側は crate の中にしか書けない**。
