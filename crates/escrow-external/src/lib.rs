...
//! # trait はドメインの語彙で切る
//!
//! [`Discover`] / [`Acquire`] / [`Transcribe`] / [`Probe`] は、ツール固有の語ではなく
//! 仕事の語で書く — 「タイムラインを列挙する」ではなく「配信元から、まだ見ていない
//! 項目を見つける」。ツールを入れ替えても、呼ぶ側は動かない。
//!
//! # 外へ出る点は受付を通る
//!
//! ツールを叩くメソッドも、引数を組み立てる関数も、プロセスを起動する `run` も、
//! crate の中でだけ見える。**外から呼べるのは [`route::Adapters`] が返す型だけ**で、
//! その中で [`through`] が順番を待つ（#13）。予算を迂回する呼び出しは、この crate の
//! 外では書けない。
//!
//! # 分け方
//!
//! 3つの層を分けて置く。それぞれ別の理由で壊れるため。
//!
//! | 層 | 形 | 壊れる理由 |
//! |---|---|---|
...
//! | 実行 | `run` 1か所 | OS 側の事情 |
//!
//! 分けてあれば、引数のテストはプロセスを起動せず argv を突き合わせるだけで済み、
//! 出力のテストは実物を固めた fixture で offline に回せる。1つの関数に混ぜると、
//! 落ちたときにどれが原因か分からない。
...
///
/// **壊れ方ごとに分けて持つ。** #5 の生存確認は「分類ではなく非対称性」で決まるので、
...
pub enum AdapterError {
    ...
    /// cookie の失効か、取り出し元のブラウザの不在。
    ///
    /// 後者だと**認証の要らない公開のものまで落ちる**ので、区別せず「設定を確かめる」へ
    /// 導く。どちらも個別の項目ではなくプラットフォーム全体の問題なので、`error` を並べず
    /// 取得を止めて人に知らせる（#5）。
    ...
    /// 相手が断った。
    ///
    /// **一時的な失敗と分けて持つ**（#13）。分けずに扱うと、相手が断った直後に同じ速さで
    /// 叩き直し、やり直しが、相手にやり直しを止められる原因そのものになる。
    #[error("{program} が拒否を受けた: {detail}")]
    Rejected {
        program: String,
        detail: String,
...
pub struct Found {
    ...
    ///
    /// 入るのは開始前の配信だけ。#13 がこの時刻に取得を予約するので、
    /// 過ぎた時刻を入れると待つ意味が無くなる。
...
///
/// 文字起こしは whisper がローカルで動くので、外へ出る点の一覧の外。飛ばす理由を書く
/// 場所も要らない。
...
pub trait Permit: Send {
    /// 呼び出しがどうなったかを伝える。相手が断っていれば、その経路がしばらく閉じる。
...
pub trait Acquire {
    ...
    /// 通し番号が増える。
    ///
    /// **どのツールが取るかは呼ぶ前に決まっている**ので、種別を受け取っても決めることが
    /// 残っていない。
    fn acquire<'a>(
        &'a self,
        url: &'a NormalizedUrl,
