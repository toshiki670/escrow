...
//!
//! 呼ぶ側（スライス・CLI・エンジン）が見るのは仕事の名前だけで、どのツールが動いたかは
//! 中に閉じる。
//!
//! # 外へ出る点はここに集まる
//!
...
/// 使えるツールを揃えたもの。
///
/// **外から呼べるのは下の3つだけ。** 中のツールを見せると呼ぶ側が対応表を迂回して
...
impl Adapters {
    ...
    /// 対応表の「検知」列。
    ///
    /// YouTube は RSS。認証が要らず、1チャンネル1回で済み、叩いてよい頻度を
    /// YouTube が公表している（#5）。X は yt-dlp ではタイムラインを列挙できないので
    /// gallery-dl。
    ...
    /// 対応表の「生存確認」列。
    ///
    /// **X 投稿には空を返す** — #5 が手段をまだ決めていない。#5 の非対称性により
    ...
    /// 生存確認のアダプタが在る種別か。
    ///
    /// アダプタそのものを組み立てずに済むので、**[`Admit`] を持たない場所からも対応表へ
    ...
    /// 1件が配信元に在るかを確かめる。
    ///
    /// **アダプタの無い種別は、観測できなかったとして返す**（#5）。#5 の非対称性が
...
enum DiscoverTool<'a> {
    ...
    ///
    /// フィードは1チャンネル1回で15件を返すが、`/watch?v=` の項目が動画か配信かは
    /// そこから決まらず、開始時刻も持たない。そこだけを1件ごとの追加取得で埋める。
    Youtube {
        rss: &'a Rss,
        ytdlp: &'a YtDlp,
    },
...
/// 巡回のたびに15回叩くことになる。リードモデルとの突き合わせは呼ぶ側の仕事なので（#1 の
/// `Item.url` の一意キー）、ここでは日時だけで切る。
///
/// 外へ出るのは、フィードが1回と、追加取得が項目ごとに1回。**それぞれ別の経路の
/// 予算に載る**（#13）。まとめて1回として数えると、配信元1本ぶんの要求が実際の
/// 15分の1に見える。
...
mod tests {
    ...
    fn liveness_checks_follow_the_table() {
        ...
        for content_type in ContentType::ALL {
            ...
            assert_eq!(
                ...
                expected,
                "アダプタの有無と、アダプタそのものが食い違う: {content_type}"
            );
        }
    }

    /// アダプタの無い種別は、外へ出ずに判定保留を返す（#5）。
    ///
    ...
    /// 全種別に取得のアダプタが在ること。種別を足したらここで気づく。
    #[test]
