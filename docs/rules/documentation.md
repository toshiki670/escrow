...
```rust
// 良い — その形にした理由
/// リードモデルのスキーマを変えたいときは移行ではなくこれを走らせる。移行に混ぜると、
/// いつの間にかリードモデルに手作業の移行が当たって作り直せなくなる。
...
**次のものは、コードの doc とコメントに加えて、Issue・PR・レビュー返信・報告にも掛かる**
...
| 肯定形で書く | 「〜できない」「〜しない」で始まる説明がある。「作る経路は2つ」と言い換えられる | 同上（能動態の系） |
