...
**Issue は合意形成の場。** 問いの内容を決めるのは発注側で、受注側が出すのは Position（案）と
Argument（賛否）。分けるのは**決定と操作**であって、書き手ではない — 書き換えの操作は発注側の
指示で受注側が行ってよい（[IBIS](https://escholarship.org/uc/item/5cj786v8)、Kunz & Rittel, 1970）。
...
### Issue の要件を越える実装が要ると分かったら、起票してから作業する
...
要求が外から入る scope creep とは別で、こちらは内側から出る。
