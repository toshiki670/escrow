...
**何をどう守っているかの正本は、守り自身。** 規約に一覧を写さない — 写すと、守りを1つ
足すたびに規約の差分が要る。規約ファイルを変える差分は `doc` / `docs` の PR に限られるので
（「[この規約を変える](amendment.md)」）、守りを足す差分とは別の PR になってしまう。

## 載せる前に破る

**「これは破れない」と書く前に、破るコードを実際に書いて落ちることを見る。** 落ちなければ、
主張のほうを実態に合わせて狭める。
