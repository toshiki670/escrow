//! 共有カーネルが膨らまないこと（#15）。
//!
//! Vertical Slice の定番の失敗は、共有部分が増えて元の `escrow-core` に戻ること。
//! 守りは2つあり、片方は構造、もう片方は可視化。
//!
//! 1. **副作用を書けなくする** — 依存が下の表のものだけなら、DB ヘルパも `async fn` も
...
///
/// **いまは空。** 空のまま残しておくと、非公開のモジュールを足しても表の編集が要る。
const INTERNAL: &[&str] = &[];
