...
# このファイルだけで、`.xcodeproj` は持たない。出来上がりは `swift/Escrow/dist/Escrow.app`。
set -eu
...
# 1. Rust。staticlib（橋）と CLI（同じ .app に入る、#3）。
...
mkdir -p "$here/Sources/EscrowFFI" "$here/Sources/EscrowBridge"
...
$bindgen --swift-sources "$lib" "$here/Sources/EscrowBridge"
...
#    前回の出来上がりは消さず上書きする。置くものは全部ここに列挙してあるので、古いものは残らない。
...
# ad-hoc 署名（#3「署名」）。Apple Silicon は署名の無い実行ファイルを起動しない。
